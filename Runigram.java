...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		Color[][] hFlipped = new Color[image.length][image[0].length];
		for (int i = 0; i < hFlipped.length; i++) {
			for (int j = 0; j < hFlipped[0].length; j++) {
				hFlipped[i][j] = image[i][image[0].length - 1 - j];
			}
		}
		return hFlipped;
	}
	...
	public static Color[][] flippedVertically(Color[][] image){
		Color[][] vFlipped = new Color[image.length][image[0].length];
		for (int i = 0; i < vFlipped.length; i++) {
			for (int j = 0; j < vFlipped[0].length; j++) {
				vFlipped[i][j] = image[image.length - 1 - i][j];
			}
		}
		return vFlipped;
	}
